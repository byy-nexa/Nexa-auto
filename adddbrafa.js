...
const autodelFile = "autodelUsers.json";
...
const bot = new TelegramBot(token, { polling: true, allowed_updates: ['message', 'callback_query', 'inline_query'] });
...
bot.on('polling_error', (err) => {
  ...
  // Jika error 409 (Conflict - bot instance sudah berjalan)
  if (err.code === 'ETELEGRAM' && err.response && err.response.statusCode === 409) {
    conflictRetryCount++;
    if (conflictRetryCount <= MAX_CONFLICT_RETRIES) {
      const waitTime = Math.pow(2, conflictRetryCount) * 1000; // exponential backoff
      console.warn(`⚠️ Bot instance conflict detected. Retry ${conflictRetryCount}/${MAX_CONFLICT_RETRIES} dalam ${waitTime}ms...`);
      setTimeout(() => {
        bot.stopPolling().then(() => {
          setTimeout(() => {
            bot.startPolling();
          }, 1000);
        }).catch(e => console.error("Error stopping polling:", e));
      }, waitTime);
    } else {
      console.error("❌ ERROR: Bot instance sudah berjalan (max retries exceeded). Hanya satu instance yang boleh aktif!");
      process.exit(1);
    }
  }
});
...
process.on('SIGINT', async () => {
  ...
  try {
    ...
    console.error('❌ Error saat menghentikan bot:', err);
    process.exit(1);
  }
});
      ...
      if (timeNow === waktu) {
        ...
        subscribers.forEach(chatId => {
          bot.sendMessage(chatId, caption, {
            ...
            reply_markup: {
              inline_keyboard: [
                [{ text: "💬 Hubungi Developer", url: "https://t.me/Nexa_Xz" }]
              ]
            }
          });
        });
...
const startImageUrl = 'https://files.catbox.moe/w568y2.jpg';
const panelImageUrl = 'https://files.catbox.moe/w568y2.jpg';
const ownerImageUrl = 'https://files.catbox.moe/w568y2.jpg';
const fiturImageUrl = 'https://files.catbox.moe/w568y2.jpg';
const cekIdImageUrl = 'https://files.catbox.moe/w568y2.jpg';
const qrisImageUrl = 'https://files.catbox.moe/4lbyz2.jpg';
...
const ytUrl = 'https://youtube.com/@rafatharcode1204';
...
// ✅ Data Informasi
const nama = '𝐀𝐮𝐭𝐨𝐎𝐫𝐝𝐞𝐫 𝐕𝟓 - Nexa_Xz✨';
const author = 't.me/Nexa_Xz';
const donasiData = {
...
bot.getMe().then(() => {
    ...
    // Kirim pesan ke owner dengan error handling
    bot.sendMessage(owner,
        `✅ 𝐁𝐨𝐭 𝐁𝐞𝐫𝐡𝐚𝐬𝐢𝐥 𝐀𝐤𝐭𝐢𝐟 𝐓𝐮𝐚𝐧 ✅\n\n` +
        `📛 𝐍𝐚𝐦𝐚: ${nama}\n` +
        `📜 𝐏𝐞𝐬𝐚𝐧: ${donasiData.pesan}`
    ).catch((err) => {
        // Ignore chat not found error
        if (err.message && err.message.includes('chat not found')) {
            console.log('ℹ️ Owner chat belum diinisialisasi. Tunggu owner mengirim pesan pertama.');
        } else {
            console.error('⚠️ Error kirim pesan:', err.message);
        }
    });
}).catch((err) => {
...
const sendStokMenu = async (chatId, messageId, username) => {
  try {
    ...
    const caption = `\`\`\`
𝐇𝐚𝐢 ${username} 👋  
〤 𝐒𝐞𝐥𝐚𝐦𝐚𝐭 𝐝𝐚𝐭𝐚𝐧𝐠 𝐝𝐢 𝐥𝐚𝐲𝐚𝐧𝐚𝐧 𝐀𝐮𝐭𝐨 𝐎𝐫𝐝𝐞𝐫 ${versi} 𝐯𝐢𝐚 𝐓𝐞𝐥𝐞𝐠𝐫𝐚𝐦.
〤 𝐏𝐞𝐬𝐚𝐧, 𝐛𝐚𝐲𝐚𝐫, 𝐝𝐚𝐧 𝐭𝐞𝐫𝐢𝐦𝐚 𝐥𝐚𝐲𝐚𝐧𝐚𝐧𝐦𝐮 𝐬𝐞𝐜𝐚𝐫𝐚 𝐨𝐭𝐨𝐦𝐚𝐭𝐢𝐬.
〤 𝐁𝐨𝐭 𝐢𝐧𝐢 𝐝𝐢𝐛𝐮𝐚𝐭 𝐨𝐥𝐞𝐡 @Nexa_Xz.  
...
∘ 🛍️ 𝐒𝐭𝐨𝐜𝐤 𝐊𝐚𝐦𝐢: /𝐬𝐭𝐨𝐤
┗━━━━━━━━━━━━━━
〤 𝐏𝐢𝐥𝐢𝐡 𝐏𝐫𝐨𝐝𝐮𝐤 𝐊𝐚𝐦𝐢 𝐃𝐢𝐛𝐚𝐰𝐚𝐡 𝐈𝐧𝐢:👇\`\`\``;
...
𝐇𝐚𝐢 ${username} 👋  
〤 𝐒𝐞𝐥𝐚𝐦𝐚𝐭 𝐝𝐚𝐭𝐚𝐧𝐠 𝐝𝐢 𝐥𝐚𝐲𝐚𝐧𝐚𝐧 𝐀𝐮𝐭𝐨 𝐎𝐫𝐝𝐞𝐫 ${versi} 𝐯𝐢𝐚 𝐓𝐞𝐥𝐞𝐠𝐫𝐚𝐦.
〤 𝐏𝐞𝐬𝐚𝐧, 𝐛𝐚𝐲𝐚𝐫, 𝐝𝐚𝐧 𝐭𝐞𝐫𝐢𝐦𝐚 𝐥𝐚𝐲𝐚𝐧𝐚𝐧𝐦𝐮 𝐬𝐞𝐜𝐚𝐫𝐚 𝐨𝐭𝐨𝐦𝐚𝐭𝐢𝐬 𝐭𝐚𝐧𝐩𝐚 𝐫𝐢𝐛𝐞𝐭.  
〤 𝐁𝐨𝐭 𝐢𝐧𝐢 𝐝𝐢𝐛𝐮𝐚𝐭 𝐨𝐥𝐞𝐡 @Nexa_Xz.  
...
∘ 🛍️ 𝐒𝐭𝐨𝐜𝐤 𝐊𝐚𝐦𝐢: /𝐬𝐭𝐨𝐤
┗━━━━━━━━━━━━━━
〤 𝐒𝐢𝐥𝐚𝐤𝐚𝐧 𝐩𝐢𝐥𝐢𝐡 𝐦𝐞𝐧𝐮 𝐝𝐢 𝐛𝐚𝐰𝐚𝐡 𝐢𝐧𝐢 😉:
\`\`\``;
    ...
    const options = {
        reply_markup: {
            inline_keyboard: [
                [
                    { text: '🛍️ 𝐒𝐭𝐨𝐤 𝐌𝐞𝐧𝐮', callback_data: 'stok_menu' },
                ],
                [
                ...
                [
                    ...
                    { text: '✨ 𝐁𝐞𝐥𝐢 𝐒𝐜 𝐈𝐧𝐢', url: 'https://t.me/Nexa_Xz' }
                ],
                [
...
\`\`\``;
    ...
    const options = {
        reply_markup: {
            inline_keyboard: [
                [
                    { text: '🛍️ 𝐒𝐭𝐨𝐫𝐞 𝐌𝐞𝐧𝐮', callback_data: 'store_menu' },
...
\`\`\`
𝐇𝐚𝐢 ${username},
〤 𝐈𝐧𝐢 𝐀𝐝𝐚𝐥𝐚𝐡 𝐌𝐞𝐭𝐨𝐝𝐞 𝐏𝐞𝐦𝐛𝐚𝐲𝐚𝐫𝐚𝐧 Nexa_Xz.
...
`\`\`\`
𝐇𝐚𝐢𝐢 ${username},
𝐈𝐧𝐢 𝐚𝐝𝐚𝐥𝐚𝐡 𝐢𝐧𝐟𝐨𝐫𝐦𝐚𝐬𝐢 𝐜𝐨𝐧𝐭𝐚𝐜𝐭 nexa.
...
\`\`\``;
    ...
    const options = {
        reply_markup: {
            inline_keyboard: [
                [
                    { text: '➕ 𝐀𝐝𝐝 𝐆𝐫𝐨𝐮𝐩', url: 'https://t.me/Nexa_Xz?startgroup=true' },
                    { text: '➕ 𝐀𝐝𝐝 𝐂𝐡𝐚𝐧𝐧𝐞𝐥', url: 'https://t.me/Nexa_Xz?startchannel=true' }
                ],
                [{ text: '🔙 𝐊𝐞𝐦𝐛𝐚𝐥𝐢', callback_data: 'starttt_menu' }]
            ]
        }
...
/𝐩𝐥𝐚𝐲 <𝐧𝐚𝐦𝐚𝐥𝐚𝐠𝐮>
/𝐛𝐫𝐚𝐭 <𝐭𝐞𝐤𝐬>
/𝐬𝐢𝐟𝐚𝐭 <𝐧𝐚𝐦𝐚>, <𝐭𝐠𝐥>, <𝐛𝐥𝐧>, <𝐭𝐡𝐧>
/𝐳𝐨𝐝𝐢𝐚𝐤 <𝐭𝐠𝐥> <𝐛𝐥𝐧> <𝐭𝐡𝐧>
/𝐚𝐫𝐭𝐢𝐧𝐚𝐦𝐚 <𝐧𝐚𝐦𝐚>
/𝐭𝐨𝐟𝐢𝐠𝐮𝐫𝐞 <𝐤𝐢𝐫𝐢𝐦 𝐟𝐨𝐭𝐨 𝐥𝐚𝐥𝐮 𝐫𝐞𝐩𝐥𝐲>
/𝐚𝐮𝐭𝐨𝐚𝐢
/𝐚𝐢𝐠𝐩𝐭 <𝐦𝐚𝐬𝐮𝐤𝐚𝐧 𝐭𝐞𝐤𝐬>
/𝐢𝐪𝐜 <𝐭𝐞𝐤𝐬>
/𝐡𝐝 <𝐤𝐢𝐫𝐢𝐦 𝐟𝐨𝐭𝐨 𝐥𝐚𝐥𝐮 𝐫𝐞𝐩𝐥𝐲>
/𝐚𝐢 <𝐦𝐚𝐬𝐮𝐤𝐚𝐧 𝐭𝐞𝐤𝐬>
...
bot.sendPhoto(chatId, startImageUrl, {
    ...
    bot.sendAudio(chatId, "https://files.catbox.moe/38j5nb.mp3", {
        title: "🎵 Sambutan Musik Nexa_Xz",
        performer: "Nexa_Xz Audio",
        caption: "✨ 𝐒𝐚𝐦𝐛𝐮𝐭𝐚𝐧 𝐌𝐮𝐬𝐢𝐤 Nexa_Xz ✨",
        parse_mode: "Markdown"
    });
  });
});
...
bot.onText(/^(\.|\#|\/)idch$/, async (msg) => {
    ...
    return bot.sendMessage(chatId, "❌ Akses ditolak! Hanya owner yang dapat menggunakan perintah ini.", {
      ...
      reply_markup: {
        inline_keyboard: [
          [{ text: "HUBUNGI ADMIN", url: "https://t.me/Nexa_Xz" }]
        ]
      }
    });
  }
...
bot.on('callback_query', async (query) => {
  ...
  if (data.startsWith("pay_dana")) {
    ...
    try {
      ...
      if (!dataRes.status) {
        console.warn(`❌ Atlantic API error: ${dataRes.message}`);
...
bot.onText(/^\/ordernokos(?:\s+(\d+))?$/, async (msg, match) => {
  ...
  try {
    ...
    const data = res.data;
    if (!data.status) {
      console.warn(`❌ Atlantic API error: ${data.message}`);
      return bot.sendMessage(chatId, "❌ Gagal membuat QRIS.");
    }
...
bot.on('callback_query', async (query) => {
    ...
    if (data === "pay") {
        ...
        try {
            ...
            const res = await axios.post('https://atlantich2h.com/deposit/create', depositData, { headers: { 'Content-Type': 'application/x-www-form-urlencoded' }, timeout: 10000 });
            const data = res.data;
            if (!data.status) {
              console.warn(`❌ Atlantic API error: ${data.message}`);
              return bot.sendMessage(chatId, `❌ Gagal membuat QRIS.\n${data.message || "Silakan coba lagi."}`);
...
bot.on("callback_query", async (query) => {
  ...
  if (data.startsWith("bayar_")) {
    ...
    try {
      ...
      const res = await axios.post("https://atlantich2h.com/deposit/create", payload, { headers: { "Content-Type": "application/x-www-form-urlencoded" }, timeout: 10000 });

      if (!res.data.status) {
        console.warn(`❌ Atlantic API error: ${res.data.message}`);
        return bot.sendMessage(chatId, `❌ Gagal membuat QRIS\n${res.data.message}`);
      }
      ...
      const checkPaymentStatus = async () => {
        ...
        try {
          const check = await axios.post("https://atlantich2h.com/deposit/status", qs.stringify({ api_key: settings.ApikeyAtlantic, id: activeOrder[userId].id }), { headers: { "Content-Type": "application/x-www-form-urlencoded" } });
          const status = check.data?.data;
          if (status && status.status !== "pending") {
            activeOrder[userId].status = false;
            clearTimeout(activeOrder[userId].timeout);

            await axios.post("https://atlantich2h.com/deposit/instant", qs.stringify({ api_key: settings.ApikeyAtlantic, id: activeOrder[userId].id, action: true }), { headers: { "Content-Type": "application/x-www-form-urlencoded" } }).catch(() => {});
...
// Jika user mengetik /deposit [angka] - via Orkut QRIS
bot.onText(/\/deposit (\d+)/, async (msg, match) => {
    ...
    if (isNaN(amount) || amount < 1000) {
        return bot.sendMessage(chatId, `🚫 Nominal tidak valid. Minimal Rp1.000. Contoh: /deposit 10000`);
    }
    ...
    try {
        ...
        const res = await fetch(`https://restapi.rafatharcode.apibotwa.biz.id/api/orkut/createpayment?apikey=${settings.apisimplebot}&amount=${amount}&codeqr=${settings.qrisorderkuota}`);
        const pay = await res.json();

        if (!pay.result || !pay.result.qrImageUrl) {
            return bot.sendMessage(chatId, `❌ Gagal membuat QRIS. Silakan coba lagi nanti.`);
        }
        ...
        await bot.sendPhoto(chatId, pay.result.qrImageUrl, { caption: teks });

        const apiUrl = `https://restapi.rafatharcode.apibotwa.biz.id/api/orkut/cekstatus?apikey=${settings.apisimplebot}&merchant=${settings.merchantidorderkuota}&keyorkut=${settings.apiorderkuota}`;
            ...
            try {
                ...
                if (result && result.amount && parseInt(result.amount) === totalAmount) {
                    ...
                    if (user) {
                        ...
                        await bot.sendMessage(chatId, '🎉 Akun Anda telah dibuat otomatis. Selamat datang!').catch(() => {});
                    }

                    fs.writeFileSync(usersFilePath, JSON.stringify(usersData, null, 2));

                    await bot.sendMessage(chatId, `✅ *PEMBAYARAN BERHASIL!*\n\n🆔 *ID:* ${pay.result.transactionId}\n💰 *JUMLAH DITAMBAHKAN:* Rp. ${amount}`).catch(() => {});
                    
                    const notif = `📢 DEPOSIT BERHASIL!\n🆔 ID: ${pay.result.transactionId}\n💰 Nominal: Rp${amount}\n👤 User: ${userId}`;
                    await bot.sendMessage(owner, notif).catch(() => {});
                }
            } catch (err) {
                console.error("Cek status error:", err.message);
            }
        ...
        if (!isTransactionComplete) {
            await bot.sendMessage(chatId, `⏳ *WAKTU PEMBAYARAN TELAH HABIS!*\nSilakan buat deposit ulang.`).catch(() => {});
        }

    } catch (error) {
        console.error("DEPOSIT ERROR:", error.message);
        await bot.sendMessage(chatId, '❌ Gagal membuat pembayaran QRIS.').catch(() => {});
    }
});
...
==============================
Terima kasih sudah order di Nexa_Xz 😁✌️
                `;
...
bot.onText(/\/tiktok (.+)/, async (msg, match) => {
    ...
    try {
        ...
        if (result.durations === 0 && result.duration === "0 Seconds") {
            ...
            if (video) {
                return bot.sendVideo(chatId, video.url, { caption: "TikTok Downloader BY Nexa_Xz" });
            }
        }
    } catch (e) {
        console.error(e);
...
bot.onText(/^(\.|\#|\/)artinama\s?(.*)/i, async (msg, match) => {
    ...
    if (!text) {
        return bot.sendMessage(chatId, `🔍 Contoh: /artinama Nexa_Xz`, {
            reply_to_message_id: reply?.message_id || msg.message_id,
            parse_mode: 'Markdown'
        });
    }
...
bot.onText(/\/installpanel1 (.+)/, async (msg, match) => {
  ...
  function handleNodeCreationInput(data, stream, domainnode, ramvps) {
    const inputs = [
      `${Tokeninstall}`, '4', 'SGP', 'Jangan Lupa Support Nexa_Xz🦅🇮🇩',
    ...
    conn.on('ready', () => {
        ...
        conn.exec(command, (err, stream) => {
            ...
            stream.on('close', (code, signal) => {
                ...
                stream.write('4\n');
                stream.write('SGP\n');
                stream.write('AutoCnode Nexa_Xz\n');
                stream.write(`${domainnode}\n`);
                stream.write('NODES\n');
                stream.write(`${ramvps}\n`);
                stream.write(`${ramvps}\n`);
...
bot.onText(/\/listsrv/, async (msg) => {
    ...
    if (userId !== owner) {
        return bot.sendMessage(chatId, "❌ Akses ditolak! Hanya owner yang dapat menggunakan perintah ini.", {
            reply_markup: {
                inline_keyboard: [[{ text: "HUBUNGI ADMIN", url: "https://t.me/Nexa_Xz" }]]
            }
        });
    }
    ...
    for (const id of [...userList]) {
        try {
            await bot.copyMessage(id, chatId, reply.message_id, {
                reply_markup: {
                    inline_keyboard: [
                        [{ text: "💬 𝐇𝐮𝐛𝐮𝐧𝐠𝐢 𝐃𝐞𝐯𝐞𝐥𝐨𝐩𝐞𝐫", url: "https://t.me/Nexa_Xz" }],
...
async function addapikey(chatId, text, targetMessageId) {
  ...
  try {
    let api = Resapiku;
    let res = await fetch(`https://restapi.Nexa_Xz.apibotwa.biz.id/api/addapikey?apikey=${api}&new_key=${newKey}&expired=${expired}`);
...
const OWNER = {
  number: "",   // nomor owner
  username: "Nexa_Xz",  // username tanpa @
...
console.log(
  chalk.green.bold(`
  ____        __       _   _                 ____          _      _  _   _  _   _  _   
 |  _ \\ __ _ / _| __ _| |_| |__   __ _ _ __ / ___|___   __| | ___| || | | || | | || |  
 | |_) / _\` | |_ / _\` | __| '_ \\ / _\` | '__| |   / _ \\ / _\` |/ _ \\ || |_| || |_| || |_ 
 |  _ < (_| |  _| (_| | |_| | | | (_| | |  | |__| (_) | (_| |  __/__   _|__   _|__   _|
 |_| \\_\\__,_|_|  \\__,_|\\__|_| |_|\\__,_|_|   \\____\\___/ \\__,_|\\___|  |_|    |_|    |_|  
`)
);

console.log(chalk.yellow.bold("✅ Bot Auto Order V5 Berhasil Tersambung!\n"));

console.log(chalk.blue.bold("📞 Contact Dev:"));
console.log(chalk.yellow.bold("   WhatsApp : +1"));
console.log(chalk.yellow.bold("   Telegram : @Nexa_Xz\n"));
